...
//Funzione di aggiornamento del equipaggiamento
...
//Funzione per inserire da tastiera o da file le info del pg
int pg_read(FILE *fp, pg_t *pgp) {
  //stdin se l'inserimento è da tastiera e non da file
  if (fscanf(fp, "%s %s %s", pgp->cod, pgp->nome, pgp->classe) < 3)
    return 0;
  //lettura delle statistiche di ogni pg
  stat_read(fp, &(pgp->b_stat));
  pgp->eq_stat = pgp->b_stat;
  pgp->equip = equipArray_init();
  return 1;
}
//Stampa della lista e dell'equipaggiamneto se presente di ogni pg
void pg_print(FILE *fp, pg_t *pgp, invArray_t invArray) {
  if (pgp == NULL)
    return;
  fprintf(fp, "-%s %s %s ", pgp->cod, pgp->nome, pgp->classe);
  //Stampa delle statistiche dei pg
  stat_print(fp, &pgp->b_stat, 1);
  //Verifica se presente un equipaggiamento
...
//Pulizia dello spazio allocato dagli equipaggiamenti
void pg_clean(pg_t *pgp) {
  if (pgp->equip!=NULL)
    equipArray_free(pgp->equip);
  pgp->equip = NULL;
}
//Funzione wrapper per la modifica degli equipaggiamenti
